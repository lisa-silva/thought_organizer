...
A small Python app that turns messy raw thoughts into organized spreadsheet rows and produces a polished professional rewrite. It includes both a command-line interface and a Streamlit interface styled with the 2031 Cognitive Luminescence palette.
...
- `processor.py` cleans raw text, creates a polished professional version, and creates a short reminder.
...
2. Produce a formal, copy-ready professional version.
3. Generate a 5-10 word reminder.
4. Route the thought into a layer and category.
5. Append the result to `storage/data.csv`.
...
After you organize a thought, the Streamlit UI shows a `Polished professional version` card and a `Copy-ready professional version` text box. Use that text box when you want to paste a cleaned, formal rewrite into another tool such as Microsoft 365 Copilot, email, notes, or a project brief.
